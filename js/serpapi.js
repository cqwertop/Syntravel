// NOTE: For security and CORS reasons, SerpApi calls are proxied through the
// local server. This file calls /api/serpapi/* endpoints.
const SERPAPI_PROXY_BASE = "/api/serpapi";

// Helper: forward client requests to the proxy server which handles auth
async function proxyGet(path, queryParams = {}) {
  const qs = new URLSearchParams(queryParams).toString();
  const url = `${SERPAPI_PROXY_BASE}${path}${qs ? `?${qs}` : ""}`;
...
async function searchFlights(from, to, departDate, returnDate, travelers) {
  try {
    ...
    return await proxyGet('/flights', params);
  } catch (error) {
...
async function searchHotels(city, checkInDate, checkOutDate, travelers) {
  try {
    ...
    return await proxyGet('/hotels', params);
  } catch (error) {
...
async function searchCars(location, startDate, endDate, pickupTime, dropoffTime) {
  try {
    ...
    return await proxyGet('/cars', params);
  } catch (error) {
